...
class DataHubListener:
    ...
    def _extract_lineage(
        ...
        headers = {
            ...
            "Authorization": "Bearer " + self.emitter._token
        }

        # check current input_urns 
        import urllib
        import json

        current_task_id = task_instance.task_id
        dag_id = task_instance.dag_id
        t_entities = urllib.parse.quote_plus(f'urn:li:dataJob:(urn:li:dataFlow:(airflow,{dag_id},prod),{current_task_id})')

        t_req_entities = requests.get(f"{self.emitter._gms_server}/entitiesV2/" + t_entities, headers=headers)

        # add table output_urns to datahub

        timestamp_now = str(int(str(int(dtx.timestamp(dtx.now()))) + '000'))
        



        dt = dtp.datetime.now()
        formatted_date = f"bigquery-{dt.year}_{dt.month:02d}_{dt.day:02d}-{dt.hour:02d}_{dt.minute:02d}_{dt.second:02d}"

        for i in task.outlets:
            uri_data = str(i.uri)

            output_urns.append("urn:li:dataset:(urn:li:dataPlatform:bigquery," + uri_data + ",PROD)")
            try:
                logger.info("Add table from outlets...")
                data1 = """{"proposal": {"entityType": "dataset", "entityUrn": "urn:li:dataset:(urn:li:dataPlatform:bigquery,""" + uri_data + """,PROD)", "changeType": "UPSERT", "aspectName": "status", "aspect": {"value": "{\\"removed\\": false}", "contentType": "application/json"}, "systemMetadata": {"lastObserved": """ + timestamp_now + """, "runId": \"""" + formatted_date + """\"}}}'"""
                requests.post(f"{self.emitter._gms_server}/aspects?action=ingestProposal", headers=headers, data=data1)
            except Exception as e:
                logger.warn(e)
                pass
    ...
    def on_task_instance_running(
        ...
        t_req_entities = requests.get(f"{self.emitter._gms_server}/entitiesV2/" + t_entities, headers=headers)
        logger.debug(f"Check current entities : {t_entities}")
        ...
        logger.debug(f"Emitted DataHub Datajob start ez: {datajob}")
        ...
        full_query = """mutation updateLineage {
            updateLineage(
                input: {
                edgesToAdd: [
                   $query   
        ...
        j_req = json.loads(t_req_entities.text)
        current_urn = j_req['aspects']['dataJobInputOutput']['value'].get('inputDatasetEdges',{})
        added_query = ''
        for x in current_urn:
            added_query += '{ upstreamUrn: "' + x['destinationUrn'] + '" downstreamUrn: "' + x['sourceUrn'] + '"} '


            logger.debug("Add current urn " + x['destinationUrn'])

        from string import Template
        rendered = Template(full_query).substitute(query=added_query)
        if len(current_urn) > 0:
            # update to ... 
            getUrl = os.getenv("DATAHUB_AIRFLOW_MIDDLEWARE_URL", "http://localhost:9004").lower()
            url = getUrl + 'api/graphql'
            
            r_post = requests.post(url=url,headers=headers,json={"operationName":"updateLineage","query":str(rendered)})
            logger.debug(rendered)
            if r_post.status_code == 200:
                r_result = json.loads(r_post.text)
                if r_result.get('errors','--') != '--':
                    # result ok
                    logger.debug("success to update lineage")
            else:
                logger.debug(rendered)
        # end test ok
    def on_task_instance_finish(
        ...
        print("RESULT DATAFLOW BEFORE RUN")
        ...
        #     capture_tags=self.config.capture_tags_info,
        #     capture_owner=self.config.capture_ownership_info,
        #     token=self.emitter._token,
        #     gms_server=self.emitter._gms_server
        # )
        ...
        try:
            ...
            getUrl = os.getenv("DATAHUB_AIRFLOW_MIDDLEWARE_URL", "http://localhost:9004").lower()
            url = getUrl + 'api/batch/push'

            for i in task.outlets:
                uri_data = str(i.uri)

                urn_bigquery = uri_data.split('.')
                if len(urn_bigquery)  == 3:
                    r = requests.post(url=url,headers={"Content-Type":"application/json"},
                                    json={"project_id":urn_bigquery[0],"target_dataset":urn_bigquery[1],"target_table":urn_bigquery[2]}
                                    )
                    print(r.text)
        except Exception:
            print("ERROR EMIT DATAFLOW")
            pass
